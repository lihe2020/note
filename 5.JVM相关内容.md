...
* 新生成的对象首先放到年轻代Eden区, 当Eden区空间满了, 出发MinorGC, 存活下来的对象移动到Survivor0区, Survivor0区满后出发执行MinorGC, Suvivor0区存活的对象移动到Servivor1区, 这样保证了一段时间内总有一个suvivor区为空。经过多次MinorGC仍然存活的对象奖移动到老年代。
...
上节课程我们学习了, Eden区属于年轻代(YoungGen), 在创建新的对象的时候, 大多数情况下, 对象现在Eden区中分配。当Eden区没有足够的空间进行分配时, 虚拟机阿静发起一次Ninor GC。
